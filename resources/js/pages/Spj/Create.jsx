...
export default function Create({ previewId = '', bidangUser = '', auth }) {
    const pptkMap = {
        ...
        MP: 'Made Herry Hermawan, S.STP., M.A.P',
    ...
    const handleSubmit = (e) => {
        ...
        // Check if no files uploaded (Kelengkapan masih kosong)
        if (selectedFilesCount === 0) {
            alert('Kelengkapan berkas masih kosong! Harap unggah minimal 1 berkas kelengkapan SPJ dalam format PDF sebelum menyimpan.');
            return;
        }
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    ...
                    <div className="flex items-center gap-2.5">
                        ...
                        <button
                            ...
                            className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-semibold bg-linear-to-r from-[#2a4574] via-[#3b5a97] to-[#4c6fc1] text-white hover:opacity-95 transition-opacity shadow-xs disabled:opacity-50 disabled:cursor-not-allowed"
                            title={selectedFilesCount === 0 ? 'Harap upload minimal 1 berkas kelengkapan untuk menyimpan SPJ' : ''}
                        >
                            ...
                            <div>
                                <div className="flex items-center gap-2">
                                    ...
                                    {selectedFilesCount === 0 ? (
                                        <span className="text-[11px] font-semibold px-2.5 py-0.5 rounded-full bg-rose-50 dark:bg-rose-950/40 text-rose-700 dark:text-rose-400 border border-rose-200 dark:border-rose-800/60 flex items-center gap-1">
                                            <AlertCircle className="w-3 h-3 text-rose-500" />
                                            <span>Wajib minimal 1 berkas</span>
                                        </span>
                                    ) : (
                                        <span className="text-[11px] font-semibold px-2.5 py-0.5 rounded-full bg-emerald-50 dark:bg-emerald-950/40 text-emerald-700 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-800/60 flex items-center gap-1">
                                            <CheckCircle2 className="w-3 h-3 text-emerald-600 dark:text-emerald-400" />
                                            <span>{selectedFilesCount} dari {masterDokumens.length} dipilih</span>
                                        </span>
                                    )}
                                </div>
                    ...
                    <div className="p-4 bg-slate-50 dark:bg-slate-800/70 border-t border-slate-100 dark:border-slate-700 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                        {selectedFilesCount === 0 ? (
                            <div className="text-xs text-rose-600 dark:text-rose-400 flex items-center gap-1.5 font-medium">
                                ...
                                <span>Kelengkapan berkas masih kosong (Wajib unggah minimal 1 dokumen PDF untuk menyimpan)</span>
                            </div>
                        ) : (
                        ...
                        <div className="flex items-center gap-2.5">
                            ...
                            <button
                                type="submit"
                                disabled={isSubmitting || selectedFilesCount === 0}
                                className="inline-flex items-center gap-1.5 px-5 py-2 rounded-xl text-xs font-semibold bg-linear-to-r from-[#2a4574] via-[#3b5a97] to-[#4c6fc1] text-white hover:opacity-95 transition-opacity shadow-xs disabled:opacity-50 disabled:cursor-not-allowed"
                                title={selectedFilesCount === 0 ? 'Harap upload minimal 1 berkas kelengkapan untuk menyimpan SPJ' : ''}
                            >
                                <Save className="w-3.5 h-3.5" />
                                <span>{isSubmitting ? 'Menyimpan...' : 'Simpan SPJ'}</span>
